...
bool solve_time_of_flight(double mass, double drag, double lift, double attack_speed, double altitude, double& time_out)
{
  ...
  const double time = 2.0 * std::sqrt(-p / 3.0) * std::cos((phi + 4.0 * M_PI) / 3.0) - b / (3.0 * a);
...
double horizontal_distance(double mass, double drag, double lift, double attack_speed, double time)
{
  ...
  const double term3 =
    time * time * time * (6.0 * drag * kGravity * l * mass - 6.0 * drag * drag * (l2 - 1.0) * attack_speed) / (36.0 * mass * mass);
  const double term4 =
    time * time * time * time *
    (-6.0 * drag * drag * kGravity * l * (1.0 + l2 + l4) * mass + 3.0 * drag * drag * drag * l2 * one_plus_l2 * attack_speed +
     6.0 * drag * drag * drag * l4 * one_plus_l2 * attack_speed) /
    (36.0 * one_plus_l2 * one_plus_l2 * mass * mass * mass);
  const double term5 =
    time * time * time * time * time *
    (3.0 * drag * drag * drag * kGravity * l * l * l * mass - 3.0 * drag * drag * drag * drag * l2 * one_plus_l2 * attack_speed) /
...
BallisticsResult compute_drop_solution(const BallisticsInput& input)
{
  ...
  if (distance < kDistanceEpsilon) {
    ...
    result.solution.maneuver_needed = true;
    result.solution.maneuver_x = input.target_x - (input.target_x - drone_x) * (h + input.acceleration_path) / distance;
    result.solution.maneuver_y = input.target_y - (input.target_y - drone_y) * (h + input.acceleration_path) / distance;
