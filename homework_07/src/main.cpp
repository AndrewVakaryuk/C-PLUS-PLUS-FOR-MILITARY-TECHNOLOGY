...
struct Coord {
  ...
  bool operator==(const Coord &other) const
  {
    ...
    return std::fabs(x - other.x) <= kCoordEps && std::fabs(y - other.y) <= kCoordEps;
  }
};
...
bool lookupAmmo(const char ammoName[], const AmmoParams *ammo, int ammoCount, double &m, double &d, double &l)
{
  for (int i = 0; i < ammoCount; i++) {
...
double computeHorizontalDistance(double m, double d, double l, double t, double V0)
{
  ...
  double term4 = t * t * t * t *
                 (-6.0 * d * d * g * l * (1.0 + l * l + l * l * l * l) * m + 3.0 * d * d * d * l * l * (1.0 + l * l) * V0 +
                  6.0 * d * d * d * l * l * l * l * (1.0 + l * l) * V0) /
                 (36.0 * (1.0 + l * l) * (1.0 + l * l) * m * m * m);
  double term5 = t * t * t * t * t * (3.0 * d * d * d * g * l * l * l * m - 3.0 * d * d * d * d * l * l * (1.0 + l * l) * V0) /
                 (36.0 * (1.0 + l * l) * m * m * m * m);

  return term1 + term2 + term3 + term4 + term5;
}

bool applyManeuver(
  double targetX, double targetY, double &xd, double &yd, double &D, double &xd_new, double &yd_new, double h, double accelerationPath)
{
  if (D < 1e-9) {
...
Coord interpolateTarget(double t, Coord **targets, int targetIndex, int timeSteps, float arrayTimeStep)
{
  ...
  return {static_cast<float>(p0.x + (p1.x - p0.x) * frac), static_cast<float>(p0.y + (p1.y - p0.y) * frac)};
}

bool computeDropPoint(Coord dronePos,
                      double zd,
                      Coord aimPoint,
                      double m,
                      double d,
                      ...
                      int debugStep)
{
  ...
  if (debugStep < DEBUG_STEPS_LIMIT) {
    DEBUG(std::fixed << std::setprecision(3) << "[DROP] step=" << debugStep << " D=" << D << " h+acc=" << (h + accelerationPath)
                     << " maneuver=" << (maneuver ? 1 : 0) << " aim=(" << aimPoint.x << "," << aimPoint.y << ")" << " fire=(" << firePoint.x
                     << "," << firePoint.y << ")" << " travelDist=" << travelDistance);
  }
  return true;
}

bool leadTargetingForTarget(Coord dronePos,
                            double zd,
                            double currentTime,
                            int targetIndex,
                            Coord **targets,
                            ...
                            double accelerationPath,
                            Coord &outFirePoint,
                            double &outTravelTime,
                            double &outImpactTime,
                            int debugStep)
{
  ...
  for (int iter = 0; iter < 10; iter++) {
    ...
    if (!computeDropPoint(dronePos, zd, aim, m, d, l, attackSpeed, accelerationPath, firePoint, travelDistance, debugStep)) {
      return false;
    }
    ...
    if (debugStep < DEBUG_STEPS_LIMIT) {
      DEBUG(std::fixed << std::setprecision(3) << "[LEAD] step=" << debugStep << " target=" << targetIndex << " iter=" << iter
                       << " predT=" << predTime << " aim=(" << aim.x << "," << aim.y << ")" << " fire=(" << firePoint.x << ","
                       << firePoint.y << ")" << " travelDist=" << travelDistance << " travelT=" << newTravel << " flightT=" << tFlight);
    }
  ...
  Coord aim = interpolateTarget(predTime, targets, targetIndex, timeSteps, arrayTimeStep);
  double travelDistance = 0.0;
  if (!computeDropPoint(dronePos, zd, aim, m, d, l, attackSpeed, accelerationPath, outFirePoint, travelDistance, debugStep)) {
    return false;
  }
...
double computeTimeToStop(DroneState state, double speed, double acceleration, double angularSpeed, double remainingTurnAngleRad)
{
  switch (state) {
    case STOPPED:
      return 0.0;
...
void decelerateThisStep(double simTimeStep, double accel, DroneState &state, double &speed, double dir, Coord &pos)
{
  if (accel <= EPS) {
    speed = 0.0;
    state = TURNING;
...
void updateDrone(double simTimeStep,
                 ...
                 Coord &pos)
{
  const double accel = (accelerationPath > EPS) ? (attackSpeed * attackSpeed) / (2.0 * accelerationPath) : 0.0;
...
bool writeSimulationJson(const char *path, const SimStep steps[], int stepCount)
{
  json out;
...
int main()
{
  ...
  LOG("Config loaded: speed=" << config.attackSpeed << " ammo=" << config.ammoName);
  ...
  if (attackSpeed <= EPS || accelerationPath <= EPS || simTimeStep <= 0.0f || arrayTimeStep <= 0.0f || angularSpeed <= 0.0f) {
  ...
  while (steps < MAX_STEPS) {
    const double linearStopTime = computeTimeToStop(state, speed, accel, static_cast<double>(angularSpeed), 0.0);
    ...
    for (int i = 0; i < targetCount; i++) {
      ...
      if (!leadTargetingForTarget(dronePos,
                                  zd,
                                  currentTime,
                                  i,
                                  targets,
    ...
    Coord aimPointNow{static_cast<float>(dronePos.x + projectileRange * std::cos(dir)),
    ...
    if (steps < DEBUG_STEPS_LIMIT) {
      DEBUG(std::fixed << std::setprecision(3) << "[STEP] step=" << steps << " t=" << currentTime << " pos=(" << dronePos.x << ","
                       << dronePos.y << ")" << " state=" << static_cast<int>(state) << " target=" << bestIndex << " drop=("
                       << bestFirePoint.x << "," << bestFirePoint.y << ")"
                       << " dist=" << std::hypot(bestFirePoint.x - dronePos.x, bestFirePoint.y - dronePos.y) << " heading=" << dir
                       << " desired=" << desiredDir);
    }
    ...
    updateDrone(dt,
                desiredDir,
                static_cast<double>(turnThreshold),
                attackSpeed,
                accelerationPath,
    ...
    Coord aimPointAfter{static_cast<float>(dronePos.x + projectileRange * std::cos(dir)),
                        static_cast<float>(dronePos.y + projectileRange * std::sin(dir))};
